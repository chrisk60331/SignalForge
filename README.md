...
🟣 [PyPI v0.7.2](https://pypi.org/project/signalforge-cli/0.7.2/) &nbsp;|&nbsp; 🐍 Python 3.11+ &nbsp;|&nbsp; 📄 MIT &nbsp;|&nbsp; ⚡ [Built on Backboard.io](https://backboard.io) &nbsp;|&nbsp; 📬 [Customer.io](https://customer.io) &nbsp;|&nbsp; 🏆 [Devpost](https://devpost.com)
...
| `DEV_TO__DEVTO_FOREM_SESSION` | `signalforge-devto` | `_Devto_Forem_Session` cookie from browser DevTools |
| `DEV_TO_REMEMBER_USER_TOKEN` | `signalforge-devto` | `remember_user_token` cookie (optional) |
| `DEV_TO_CURRENT_USER` | `signalforge-devto` | `current_user` cookie (optional) |
...
### `signalforge-devto` — dev.to challenge submitters

Walk all active and previous [dev.to challenges](https://dev.to/challenges), scrape every submission article, extract author GitHub usernames from the dev.to API, look up real emails via GitHub, and store everything in SQLite.

**How it works:**
1. Scrapes `https://dev.to/challenges` for active + previous challenges
2. Fetches each challenge detail page to find the submission tag (e.g. `#418challenge`)
3. Paginates `GET https://dev.to/api/articles?tag={tag}` — no auth needed for articles
4. Extracts `user.github_username` directly from the API response
5. Looks up email via GitHub API (3 strategies: profile → commits → push events)
6. Deduplicates by `(challenge_tag, dev.to username)` — only new submitters trigger events

```bash
# First-time scrape: active + previous challenges, enrich emails
signalforge-devto

# Active challenges only
signalforge-devto --state active

# Skip email enrichment (fast scan)
signalforge-devto --no-email

# Emit Customer.io events for new submitters
signalforge-devto --emit-events

# Emit all unsent events in the DB (no scraping)
signalforge-devto --emit-unsent

# Re-scrape already-seen challenges (picks up new submissions)
signalforge-devto --rescrape --emit-events

# Cap submissions per challenge
signalforge-devto --max-submissions 50
```

**Required env** — copy from browser DevTools → Application → Cookies → `dev.to`:

```bash
DEV_TO__DEVTO_FOREM_SESSION=<value of _Devto_Forem_Session>
DEV_TO_REMEMBER_USER_TOKEN=<value of remember_user_token>   # optional
DEV_TO_CURRENT_USER=<value of current_user>                  # optional
```
...
| Flag | Default | Description |
|---|---|---|
| `--state` | `active`+`previous` | `active` / `previous` / `upcoming` (repeatable) |
| `--max-submissions N` | `0` (unlimited) | Cap submissions per challenge |
| `--no-email` | off | Skip email enrichment entirely |
| `--emit-events` | off | Emit `devto_challenge` Customer.io events for new submitters |
| `--emit-unsent` | off | Just emit — no scraping |
| `--rescrape` | off | Re-scrape already-scraped challenges |
| `--db PATH` | `devpost_harvest.db` | SQLite path |

**SQLite storage** — reuses the shared `participants` table:
- `hackathon_url` = `devto:challenge:{tag}` (e.g. `devto:challenge:418challenge`)
- `hackathon_title` = challenge title
- `username` = dev.to username
- `profile_url` = `https://dev.to/{username}`
- `github_url` = extracted from dev.to API user object
- `specialty` = article URL
