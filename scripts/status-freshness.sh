...
SELECT 'last_h_days=' || COALESCE((CURRENT_DATE - MAX(DATE(last_scraped_at))), 0) FROM hackathons;
...
SELECT 'last_f_days=' || COALESCE((CURRENT_DATE - MAX(DATE(last_seen_at))), 0) FROM participants WHERE hackathon_url LIKE 'github:forks:%';
...
SELECT 'last_s_days=' || COALESCE((CURRENT_DATE - MAX(DATE(last_seen_at))), 0) FROM participants WHERE hackathon_url LIKE 'github:search:%';
...
SELECT 'last_dt_days=' || COALESCE((CURRENT_DATE - MAX(DATE(last_scraped_at))), 0) FROM devto_challenges;
SQL
  )" 2>/dev/null || true
fi
...
SELECT 'last_v_days=' || COALESCE((CURRENT_DATE - MAX(DATE(imported_at))), 0) FROM rb2b_visitors;
SQL
  )" 2>/dev/null || true
fi
